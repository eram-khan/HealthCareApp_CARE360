...
const CalendarStep = ({
  ...
  const isSlotBooked = (slot) => {
    if (!selectedDate || !Array.isArray(bookedSlots)) return false;
  ...
  return (
    <div className="space-y-8">
        <div>
            <h3 className="text-2xl font-bold text-gray-900 mb-6">{t('bookingFlow.select_date_time')}</h3>
            <div className="grid md:grid-cols-2 gap-8">
               <div>
                <Label className="text-base font-semibold mb-4 block">{t('bookingFlow.choose_date')}</Label>
              ...
              <span className="text-sm font-normal text-gray-500 ml-2">
                      ({availableSlots.length} {t('bookingFlow.slots_available')})
                    </span>
              }
                </Label>
            ...
            <div className="space-y-4">
              ...
              <Button
                ...
                onClick={() => setShowMoreSlots(!showMoreSlots)}>
                          ...
                          {showMoreSlots ? t('bookingFlow.show_less') : `+ ${availableSlots.length - 10} ${t('bookingFlow.show_more')}`}
                        </Button>
              }
            ...
            <div className="text-center py-12 text-gray-500">
                      ...
                      <p className="text-sm">{t('bookingFlow.diff_date')}</p>
                      </div> :


            <div className="text-center py-12 text-gray-500">
                      <CalendarIcon className="w-16 h-16 mx-auto mb-4 opacity-30" />
                      <p className="text-sm">{t('bookingFlow.select_to_view')}</p>
                      </div>
            }
              </div>
            </div>
