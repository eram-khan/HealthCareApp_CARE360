...
const PatientDashboardContent = () => {
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="text-center md:text-left">
                <div className="flex justify-center gap-2 text-sm text-gray-600">
                  <span className="font-semibold">{t("patientDashboard.fee")}</span>
                  <p>₹{appointment.doctorId?.fees}</p>
                </div>

                {appointment.symptoms &&
              <div className="flex justify-center gap-2 text-sm text-gray-600 mt-1">
                    <span className="font-semibold">{t("patientDashboard.symptoms")}</span>
              ...
              <PrescriptionViewModal
                ...
                <Button
                  ...
                  className="text-green-700 border-green-200 hover:bg-green-50">
                        ...
                        {t('patientDashboard.view_prescription')}
                      </Button>
                } />
  ...
  const EmptyState = ({ tab }) => {
    const emptyStates = {
      upcoming: {
        icon: Clock,
        title: t("patientDashboard.no_upcoming_title"),
        description: t("patientDashboard.no_upcoming_desc"),
        showBookButton: true
      },
      past: {
        icon: FileText,
        title: t("patientDashboard.no_past_title"),
        description: t("patientDashboard.no_past_desc"),
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-md md:text-3xl font-bold text-gray-900">
                {t('patientDashboard.my_appointment')}
              </h1>
              <p className="text-xs md:text-lg text-gray-600">
                {t('patientDashboard.manage_appointments')}
              </p>
            </div>
            ...
            <div className="flex items-center space-x-4 ">
              <Link href="/patient/chat">
                <Button variant="outline" className="text-blue-600 border-blue-200 hover:bg-blue-50">
                  ...
                  <span className="hidden md:block">{t('patientDashboard.chats_ai')}</span>
                </Button>
              </Link>
              <Link href="/doctor-list">
                <Button>
                  <Calendar className="w-4 h-4 mr-2 " />
                  Book <span className="hidden md:block">{t('patientDashboard.book_new')}</span>
                </Button>
              </Link>
            </div>
          </div>
          ...
          <Tabs
            ...
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger
                ...
                <span>{t('patientDashboard.upcoming')} ({tabCounts.upcoming})</span>
              </TabsTrigger>
              <TabsTrigger value="past" className="flex items-center space-x-2">
                <Calendar className="w-4 h-4" />
                <span>{t('patientDashboard.past')} ({tabCounts.past})</span>
              </TabsTrigger>
            </TabsList>
